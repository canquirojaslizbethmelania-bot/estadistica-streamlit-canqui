...
st.title("📊 PROCESAMIENTO ESTADISTICO - DATOS DE VENTAS")
...
st.write("Lectura del archivo ventas_tienda.csv")
...
st.write("Variable cualitativa: categoria_producto")

# =========================================================
# TABLA DE FRECUENCIA CUALITATIVA
# =========================================================

frecuencia_carrera = df["categoria_producto"].value_counts()

tabla_cualitativa = pd.DataFrame({
    "Categoria": frecuencia_carrera.index,
...
ax1.set_title(
    "DISTRIBUCION POR CATEGORIA DE PRODUCTO",
    fontsize=14,
    fontweight="bold"
)
...
ax2.pie(
    tabla_cualitativa["fi"],
    labels=tabla_cualitativa["Categoria"],
    autopct="%1.1f%%",
    startangle=90
)
...
st.write("Variable cuantitativa discreta: monto_venta")

# DATOS NO AGRUPADOS: MONTO VENTA
# =========================================================
...
tabla_discreta = pd.DataFrame({
    "Monto Venta": frecuencia_nota.index,
...
media = df["monto_venta"].mean()
mediana = df["monto_venta"].median()
moda = df["monto_venta"].mode()[0]
varianza = df["monto_venta"].var()
desviacion = df["monto_venta"].std()

valor_maximo_nota = df["monto_venta"].max()
valor_minimo_nota = df["monto_venta"].min()
...
markerline, stemlines, baseline = ax3.stem(
    tabla_discreta["Monto Venta"],
    tabla_discreta["fi"],
    basefmt="-" # <-- SOLO QUITAMOS LO QUE DABA ERROR, LO DEMAS IGUAL
)
...
ax3.set_title(
    "DISTRIBUCION DE MONTOS DE VENTA",
    fontsize=14,
    fontweight="bold"
)

ax3.set_xlabel("Montos")
...
st.write("Variable cuantitativa continua: edad_cliente")

# =========================================================
# PARAMETROS
# =========================================================

n = len(df["edad_cliente"])

valor_maximo = df["edad_cliente"].max()
valor_minimo = df["edad_cliente"].min()
